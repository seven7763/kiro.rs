 ...
 *
 * 密码语义（避免回显已存密码）：
 * - proxyPassword 省略 / undefined → 保留原密码（新建则无）
 * - proxyPassword === "" → 清空密码
 * - proxyPassword 非空 → 更新密码
 */
export interface UpsertCredentialGroupRequest {
  id: string
  /** 代理 URL；省略 / "" / "direct" 表示该组直连。SOCKS5 用 socks5://host:port */
  proxyUrl?: string
  proxyUsername?: string

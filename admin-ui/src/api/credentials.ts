...
// 列出所有凭据分组
export async function listCredentialGroups(): Promise<CredentialGroupStatusItem[]> {
  const { data } = await api.get<CredentialGroupStatusItem[]>('/config/credential-groups')
  return data
}

// 新建 / 更新凭据分组（按 id upsert）
export async function upsertCredentialGroup(
  req: UpsertCredentialGroupRequest
): Promise<SuccessResponse> {
  const { data } = await api.post<SuccessResponse>('/config/credential-groups', req)
  return data
}

// 删除凭据分组（挂靠该组的凭据回落本机直连）
export async function deleteCredentialGroup(id: string): Promise<SuccessResponse> {
  const { data } = await api.delete<SuccessResponse>(
    `/config/credential-groups/${encodeURIComponent(id)}`
  )
  return data
}

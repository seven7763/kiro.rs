...
import {
  ...
  upsertCredentialGroup,
  deleteCredentialGroup,
} from '@/api/credentials'
import type { AddCredentialRequest, UpsertCredentialGroupRequest } from '@/types/api'
...
// 新建 / 更新凭据分组
export function useUpsertCredentialGroup() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (req: UpsertCredentialGroupRequest) => upsertCredentialGroup(req),
    onSuccess: () => {
      // 分组改动会影响凭据快照里的 proxySource，两个查询都失效

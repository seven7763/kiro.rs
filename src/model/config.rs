...
pub struct Config {
    ...
    /// 西文字符 token 估算权重（默认 1.85）。
    ///
    /// 本地估算器把字符折算成 token：CJK 等非西文按 4.0 单位/字符，西文按本权重单位/字符，
    /// 再 ÷4 得 token。历史默认西文 1.0（即 4 字符=1 token），但实测纯英文真实约
    /// 2.1 字符/token，导致英文 input/cache_read 系统性低估约 1.86×（NewAPI 计费少算近半）。
    ///
    /// 默认 1.85 来自实测点（19200 英文字符 → Anthropic 真实 8944 token）。配 `1.0` 可回退
    /// 旧行为。仅影响**本地估算**；配了 `countTokensApiUrl` 时走外部精确计数、本权重不参与。
    #[serde(default = "default_western_char_weight")]
    pub token_western_char_weight: f64,
...
/// 西文字符 token 估算权重默认值。见 [`Config::token_western_char_weight`]。
fn default_western_char_weight() -> f64 {
    1.85
}
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            token_western_char_weight: default_western_char_weight(),

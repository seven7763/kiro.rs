...
/// 在分组列表中按 id 解析凭据所属分组（空字符串/None 视为未分组）。
///
/// 作用于运行时可变的分组快照，让分组改动无需重启即生效。
fn resolve_group<'a>(
    groups: &'a [CredentialGroupConfig],
    group_id: Option<&str>,
) -> Option<&'a CredentialGroupConfig> {
...
pub struct MultiTokenManager {
    ...
    /// 凭据分组（运行时可修改）。分组代理改动需即时生效，因此从 `config` 抽出
    /// 成独立可变状态（对标 `load_balancing_mode`），Admin 新建/改/删分组后下次
    /// 取凭据即走新出口，无需重启。
    credential_groups: Mutex<Vec<CredentialGroupConfig>>,
...
impl MultiTokenManager {
    ...
    pub fn new(
        ...
        let credential_groups = config.credential_groups.clone();
        let manager = Self {
            config,
            proxy,
            ...
            credential_groups: Mutex::new(credential_groups),
    ...
    fn effective_proxy_for(&self, credentials: &KiroCredentials) -> Option<ProxyConfig> {
        let groups = self.credential_groups.lock();
        let group = resolve_group(&groups, credentials.group.as_deref());
        credentials.effective_proxy_with_group(group, self.proxy.as_ref())
    }

    /// 解析凭据的有效代理（供 provider 在锁外调用，分组读运行时可变状态）。
    pub fn effective_proxy_for_credential(
        &self,
        credentials: &KiroCredentials,
    ) -> Option<ProxyConfig> {
        self.effective_proxy_for(credentials)
    }

    /// 凭据是否有显式出口路由（凭据级 proxy/direct 或有效分组），分组读运行时状态。
    pub fn has_explicit_proxy_route_for(&self, credentials: &KiroCredentials) -> bool {
        let groups = self.credential_groups.lock();
        let group = resolve_group(&groups, credentials.group.as_deref());
        credentials.has_explicit_proxy_route(group)
    }
...
mod tests {
    ...
    /// 分组热更新 CRUD：upsert 后立即生效（effective_proxy 走新出口）、
    /// 落盘 config.json、删除时解除挂靠凭据并回落直连。
    #[test]
    fn test_credential_group_crud_hot_update_and_persist() {
        let dir = std::env::temp_dir().join(format!("kiro-group-crud-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let config_path = dir.join("config.json");
        let creds_path = dir.join("credentials.json");
        std::fs::write(&config_path, r#"{"host":"127.0.0.1"}"#).unwrap();

        let config = Config::load(&config_path).unwrap();
        let mut cred = KiroCredentials::default();
        cred.id = Some(1);
        cred.refresh_token = Some("a".repeat(150));
        cred.group = Some("socks-a".to_string());

        let manager =
            MultiTokenManager::new(config, vec![cred], None, Some(creds_path), true).unwrap();

        // upsert 新建分组 → 立即生效，凭据 #1 的有效代理应走分组的 socks5
        manager
            .upsert_group(CredentialGroupConfig {
                id: "socks-a".to_string(),
        ...
        assert_eq!(
            manager.effective_proxy_for_credential(&creds_snapshot),
            Some(ProxyConfig::new("socks5://127.0.0.1:1080")),
            "upsert 分组后应立即按新出口解析代理"
        );
        assert!(manager.has_explicit_proxy_route_for(&creds_snapshot));

        // 落盘后重载 config 应能看到分组
        let persisted = Config::load(&config_path).unwrap();
        assert_eq!(persisted.credential_groups.len(), 1);
        assert_eq!(persisted.credential_groups[0].id, "socks-a");

        // upsert 同 id → 更新（改成直连），mutex 与磁盘都应反映
        manager
            .upsert_group(CredentialGroupConfig {
                id: "socks-a".to_string(),
        ...
        // 删除分组 → 返回被解除挂靠的凭据 id，凭据 group 置空（回落本机直连）
        let detached = manager.remove_group("socks-a").unwrap();
        assert_eq!(detached, vec![1], "挂靠该组的凭据 #1 应被解除");
        assert!(manager.list_groups().is_empty());
        {
            let entries = manager.entries.lock();
            assert_eq!(entries[0].credentials.group, None, "凭据 group 应被置空");
        }

        // config.json 也应清空分组
        let persisted = Config::load(&config_path).unwrap();
        assert!(persisted.credential_groups.is_empty());

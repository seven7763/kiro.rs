//! Admin CRUD 操作（snapshot / 启停 / 优先级 / 增删 / 余额查询 / 负载均衡模式）
...
impl MultiTokenManager {
    ...
    pub fn snapshot(&self) -> ManagerSnapshot {
        ...
        ManagerSnapshot {
            entries: entries
                .iter()
                .map(|e| {
                    let group = resolve_group(&groups, e.credentials.group.as_deref());
                    let (proxy_source, effective_proxy_url) = e
                        .credentials
                        .effective_proxy_display(group, self.proxy.as_ref());
                    CredentialEntrySnapshot {
                        ...
                        directory_key: e.directory_key.clone(),
                    }
                })
                .collect(),
            current_id,
    ...
    /// 列出当前运行时的凭据分组（Admin API）。
    pub fn list_groups(&self) -> Vec<CredentialGroupConfig> {
        self.credential_groups.lock().clone()
    }

    /// 新增或更新一个凭据分组（Admin API，按 `id` upsert）。
    ///
    /// 改动即时生效：下次取凭据时 [`Self::effective_proxy_for`] 读 mutex 即走新出口，
    /// 无需重启。先在锁外算出新快照、持久化成功后再 commit 到 mutex，保证内存与磁盘一致。
    pub fn upsert_group(&self, group: CredentialGroupConfig) -> anyhow::Result<()> {
        let id = group.id.trim();
        if id.is_empty() {
            anyhow::bail!("分组 ID 不能为空");
        }

        let new_groups = {
            let groups = self.credential_groups.lock();
            let mut next = groups.clone();
            if let Some(existing) = next.iter_mut().find(|g| g.id == group.id) {
        ...
        self.persist_credential_groups(&new_groups)?;
        *self.credential_groups.lock() = new_groups;

        tracing::info!("凭据分组已更新: {}", group.id);
        Ok(())
    }

    /// 删除一个凭据分组（Admin API）。
    ///
    /// 删除策略：先把所有挂靠该分组的凭据 `group` 置空（回落本机直连）并落盘，
    /// 再从运行时分组移除该组并持久化分组。返回被解除挂靠的凭据 ID 列表，
    /// 供调用方提示用户。分组不存在时按幂等处理（detached 为空）。
    pub fn remove_group(&self, id: &str) -> anyhow::Result<Vec<u64>> {
        let id = id.trim();
        if id.is_empty() {
            anyhow::bail!("分组 ID 不能为空");
        }

        // 1. 解除所有挂靠该分组的凭据（回落本机直连），记录受影响的 ID
        let detached: Vec<u64> = {
            let mut entries = self.entries.lock();
            let mut ids = Vec::new();
            for entry in entries.iter_mut() {
                if entry.credentials.group.as_deref() == Some(id) {
                    entry.credentials.group = None;
                    ids.push(entry.id);
                }
            }
            ids
        ...
        // 2. 凭据 group 改动落盘（仅多凭据格式生效；单凭据格式静默跳过）
        ...
        // 3. 从运行时分组移除并落盘
        let new_groups = {
            let mut groups = self.credential_groups.lock();
